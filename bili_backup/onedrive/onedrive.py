...
LastEditors: Kumo
LastEditTime: 2024-06-20 18:50:16
Description: 
'''
...
class OnedriveManager(BaseRequest):
    ...
    def upload_large_file(self,filepath, filename, verbose_prefix="", verbose_suffix="") -> bool:
        ...
        with open(filepath, 'rb') as f:
            ...
            for i in tqdm(range(0, size, chunk_size), desc=f"{verbose_prefix}{verbose_suffix}", unit="B", unit_scale=True):
                # print(i/size,i,size)
                chunk_data = data[i:i+chunk_size]
                r = self._http.put(upload_session['uploadUrl'], headers={'Content-Length': str(len(chunk_data)),
